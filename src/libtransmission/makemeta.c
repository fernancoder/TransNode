...
//CODIGO NUEVO ********************
void
poemRealMakeMetaInfo (tr_metainfo_builder *   builder,
                      const char *            outputFile,
                      const tr_tracker_info * trackers,
                      int                     trackerCount,
                      const char *            comment,
...
tr_metainfo_builder* poemMakeMetaInfo (const char * filePath, const char * torrentPath, int pieceSize)
{
    int i;
    struct FileList * files;
    struct FileList * walk;
...
*/
    ret->isSingleFile = true;

    /* build a list of files containing topFile and,
       if it's a directory, all of its children */
    {
        char * dir = tr_dirname (filePath);
        char * base = tr_basename (filePath);
        files = getFiles (dir, base, NULL);
        tr_free (base);
        tr_free (dir);
    }
    ...
    if ( pieceSize == 0 )
    	...
    	ret->pieceSize = pieceSize;
    ret->pieceCount = (int)(ret->totalSize / ret->pieceSize);
    if (ret->totalSize % ret->pieceSize)
        ++ret->pieceCount;

    const char * comment = NULL;
    poemRealMakeMetaInfo(ret, torrentPath, NULL, 0, comment, false);

    //tr_realMakeMetaInfo(ret);

    return ret; //???? Para liberar... se podróia hacer aquí y que retorne void
}
//CODIGO NUEVO ********************
